...
while not shut_down_robot:
    try:
        if robot_is_ready:
            ...
            data = stdout.readlines()  # Read room name from server.

            # Get room name from top of list, remove newline from string, and make lowercase.
            room = data[0][:-1].lower() 
            print(room)
            time.sleep(2)
            ...
            # Check if robot is trying to go to the dock when it's already docked. If not, star moving!
